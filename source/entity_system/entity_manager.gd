...
## [return] 加载的实体
func load_entity(entity_id: StringName, scene_path: String, load_mode: CoreSystem.ResourceManager.LOAD_MODE = CoreSystem.ResourceManager.LOAD_MODE.IMMEDIATE) -> PackedScene:
	if _entity_resource_cache.has(entity_id):
