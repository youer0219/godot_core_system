...
## 输入配置适配器
var _config_adapter: InputConfigAdapter
...
func setup_config(config_manager: ConfigManager) -> void:
	_config_adapter = InputConfigAdapter.new(config_manager)
	_config_adapter.config_updated.connect(_on_config_updated)
	
	# 如果配置已加载，立即应用
	if config_manager.is_loaded():
		_on_config_updated(_config_adapter.get_input_config().get_config())

## 配置更新回调
## [param config] 新配置
func _on_config_updated(config: Dictionary) -> void:
	# 应用动作映射
	var action_mappings = config.get("action_mappings", {})
	for action in action_mappings:
		update_action_mapping(action, action_mappings[action])
	
	# 应用轴映射
	var axis_mappings = config.get("axis_mappings", {})
	for axis in axis_mappings:
		update_axis_mapping(axis, axis_mappings[axis])
	
	# 应用设备映射
	var device_mappings = config.get("device_mappings", {})
	for device_id_str in device_mappings:
		var device_id = str(device_id_str).to_int()
		if _devices.has(device_id):
			_devices[device_id].apply_mapping(device_mappings[device_id_str])
	
	# 应用输入设置
	var settings = config.get("input_settings", {})
	_virtual_axis.set_deadzone(settings.get("deadzone", 0.2))
	_virtual_axis.set_sensitivity(settings.get("axis_sensitivity", 1.0))

## 更新动作映射
## [param action] 动作名称
## [param events] 事件列表
func update_action_mapping(action: String, events: Array) -> void:
	# 先移除现有的映射
	if InputMap.has_action(action):
		InputMap.action_erase_events(action)
	else:
	...
	# 更新配置
	if _config_adapter:
		_config_adapter.set_action_mapping(action, events)

## 更新轴映射
## [param axis] 轴名称
## [param mapping] 轴映射数据
func update_axis_mapping(axis: String, mapping: Dictionary) -> void:
	_virtual_axis.set_axis_mapping(axis, mapping)
	
	# 更新配置
	if _config_adapter:
		_config_adapter.set_axis_mapping(axis, mapping)

## 从数据创建输入事件
## [param data] 事件数据
## [return] 输入事件
func create_input_event_from_data(data: Dictionary) -> InputEvent:
...
## [param event] 输入事件
## [return] 事件数据
func get_event_data(event: InputEvent) -> Dictionary:
	var data = {
...
func save_config() -> void:
	if _config_adapter:
		_config_adapter.save_config()

## 重置为默认配置
func reset_to_default() -> void:
	if _config_adapter:
		_config_adapter.reset_to_default()